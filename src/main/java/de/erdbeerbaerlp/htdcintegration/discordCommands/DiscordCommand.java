...
public abstract class DiscordCommand extends CommandDataImpl {
    ...
    public boolean canUserExecuteCommand(User user) {
        final Member m = DiscordPlugin.getInstance().discord.getMemberById(user.getIdLong());
        if (m == null) return false;
        return !this.adminOnly() || DiscordPlugin.getInstance().discord.hasAdminRole(m.getRoles());
    }
    ...
    public final String getPlayerNotFoundMsg(String playerName) {
        return DiscordPlugin.getInstance().messages.get().playerNotFound.replace("%playername%", playerName);
    }
